...
package client // import "WebParser/http/client"
	...
	if err != nil {
		if uerr, ok := err.(*url.Error); ok {
			...
			case x509.CertificateInvalidError, x509.HostnameError:
				...
				err = fmt.Errorf(errInvalidCertificate, uerr.Err)
			case *net.OpError:
				err = fmt.Errorf(errNetworkOperation, uerr.Err)
			case net.Error:
				nerr := uerr.Err.(net.Error)
				if nerr.Timeout() {
					err = fmt.Errorf(errRequestTimeout, c.ClientTimeout)
				}
			}
		}
